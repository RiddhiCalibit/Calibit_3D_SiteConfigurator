    ...
    if (format === "pdf") {
      ...
      const generatePDF = (
        ...
        if (viewSnapshots && viewSnapshots.length > 0) {
          ...
          // Each view (label + subtitle + image) is rendered at full page
          // width (182mm) stacked vertically, matching the reference design.
          const imgX = 14;
          const imgW = 182;
          const imgH = 82; // image frame height
          const labelH = 7; // teal label line
          const subtitleH = 5; // grey subtitle line
          const blockPad = 8; // gap between consecutive view blocks
          const pageH = 297; // A4 height in mm
          const marginBottom = 14;

          let curY = 30;

          viewSnapshots.forEach((view) => {
            const blockH = labelH + subtitleH + imgH;

            // If this block won't fit on the current page, start a new one
            if (curY + blockH > pageH - marginBottom) {
              doc.addPage();
              doc.setFillColor(...dark);
              doc.rect(0, 0, 210, 24, "F");
              doc.setFontSize(12);
              doc.setFont("helvetica", "bold");
              doc.setTextColor(...white);
              doc.text("3D View Perspectives (cont.) — " + projectName, 14, 16);
              curY = 30;
            }
            ...
            doc.setFont("helvetica", "bold");
            doc.setTextColor(...teal);
            doc.text(view.label.toUpperCase(), imgX, curY + labelH - 1);

            // Subtitle (pitch / bearing)
            doc.setFontSize(7.5);
            doc.setFont("helvetica", "normal");
            doc.setTextColor(120, 130, 140);
            doc.text(view.subtitle, imgX, curY + labelH + subtitleH - 2);

            // Image with border
            const imageY = curY + labelH + subtitleH;
            doc.setDrawColor(220, 225, 230);
            doc.setLineWidth(0.3);
            doc.rect(imgX, imageY, imgW, imgH);
            try {
              doc.addImage(
                view.dataUrl,
                "PNG",
      ...
      // 2. Top-down plan view  (pitch: 0, bearing: 0) — fitted to boundary
      // 3. Isometric view      (pitch: 45, bearing: 45) — fitted to boundary
      ...
      > => {
        return new Promise((resolve) => {
          ...
          // Build LngLatBoundsLike from the site boundary polygon
          const buildBoundaryBounds = ():
            ...
            let minLng = Infinity,
              ...
              minLat = Infinity,
              maxLat = -Infinity;
            for (const [bLng, bLat] of boundary) {
          ...
          const bounds = buildBoundaryBounds();
          ...
          // Step 2: Top-down plan view — zoom into boundary
          const fitAndCapture2 = () => {
            if (bounds) {
              map.fitBounds(bounds, {
                pitch: 0,
                bearing: 0,
                // Tighter padding — boundary and equipment fill the frame
                padding: 50,
                duration: 600,
                maxZoom: 20,
              });
            ...
            map.once("idle", () => {
              ...
              // Step 3: Isometric view — zoom into boundary with 3D pitch
              if (bounds) {
                map.fitBounds(bounds, {
                  pitch: 45,
                  bearing: 45,
                  // Same padding/zoom as top-down so both views show at the same scale
                  padding: 50,
                  duration: 700,
                  maxZoom: 20,
                });
      ...
      // ── Fetch Mapbox Static map (or render plain canvas) ────────────────
      // In plain mode we skip the tile API entirely and draw a clean white
      // canvas with just the boundary + equipment overlays.
      const isPlainMode = state.mapStyle === "plain";

      if (isPlainMode) {
        // Build a plain white site-map canvas
        const plainCanvas = document.createElement("canvas");
        plainCanvas.width = 1200;
        plainCanvas.height = 600;
        const pCtx = plainCanvas.getContext("2d")!;

        // White background
        pCtx.fillStyle = "#ffffff";
        pCtx.fillRect(0, 0, plainCanvas.width, plainCanvas.height);

        // Grid lines for orientation
        pCtx.strokeStyle = "#e5e7eb";
        pCtx.lineWidth = 1;
        for (let gx = 0; gx < plainCanvas.width; gx += 60) {
          pCtx.beginPath();
          pCtx.moveTo(gx, 0);
          pCtx.lineTo(gx, plainCanvas.height);
          pCtx.stroke();
        }
        for (let gy = 0; gy < plainCanvas.height; gy += 60) {
          pCtx.beginPath();
          pCtx.moveTo(0, gy);
          pCtx.lineTo(plainCanvas.width, gy);
          pCtx.stroke();
        }

        // Draw overlays only when a boundary bbox is available
        if (mapBbox) {
          const lngSpan = mapBbox.maxLng - mapBbox.minLng;
          const latSpan = mapBbox.maxLat - mapBbox.minLat;
          const pPad = 80;
          const contentW = plainCanvas.width - pPad * 2;
          const contentH = plainCanvas.height - pPad * 2;

          const proj = (lngVal: number, latVal: number) => ({
            x: pPad + ((lngVal - mapBbox!.minLng) / lngSpan) * contentW,
            y: pPad + ((mapBbox!.maxLat - latVal) / latSpan) * contentH,
          });

          // Draw boundary fill + stroke
          if (boundary.length > 1) {
            pCtx.save();
            pCtx.beginPath();
            boundary.forEach((pt: [number, number], i: number) => {
              const { x, y } = proj(pt[0], pt[1]);
              i === 0 ? pCtx.moveTo(x, y) : pCtx.lineTo(x, y);
            });
            pCtx.closePath();
            pCtx.fillStyle = "rgba(20, 184, 166, 0.12)";
            pCtx.strokeStyle = "#14b8a6";
            pCtx.lineWidth = 3;
            pCtx.fill();
            pCtx.stroke();
            pCtx.restore();
          ...
          // Draw starting point
          if (startPointLngLat) {
            const { x, y } = proj(startPointLngLat[0], startPointLngLat[1]);
            const r = 9;
            pCtx.save();
          ...
          equipmentMapItems.forEach((item, index) => {
            const { x, y } = proj(item.lng, item.lat);
            const radius = 14;
            pCtx.save();

            // Name label box above the pin
            ...
            const boxX = Math.max(
              4,
              Math.min(x - boxW / 2, plainCanvas.width - boxW - 4),
            );
            const boxY = Math.max(4, y - radius - boxH - 5);

            const rr = 4;
            pCtx.fillStyle = "rgba(15, 23, 42, 0.88)";
            pCtx.beginPath();
            pCtx.moveTo(boxX + rr, boxY);
            pCtx.lineTo(boxX + boxW - rr, boxY);
            pCtx.quadraticCurveTo(boxX + boxW, boxY, boxX + boxW, boxY + rr);
            pCtx.lineTo(boxX + boxW, boxY + boxH - rr);
            ...
            pCtx.lineTo(boxX + rr, boxY + boxH);
            pCtx.quadraticCurveTo(boxX, boxY + boxH, boxX, boxY + boxH - rr);
            pCtx.lineTo(boxX, boxY + rr);
            pCtx.quadraticCurveTo(boxX, boxY, boxX + rr, boxY);
            pCtx.closePath();
            pCtx.fill();
            ...
            pCtx.fillText(label, boxX + boxW / 2, boxY + boxH / 2);
            ...
            pCtx.textAlign = "center";
            pCtx.textBaseline = "middle";
            pCtx.fillText(String(index + 1), x, y);
        ...
        generatePDF(
          plainCanvas.toDataURL("image/png"),
          viewSnapshots.length > 0 ? viewSnapshots : undefined,
        );
      } else if (mapboxToken && lng !== 0) {
        const img = new Image();
        img.crossOrigin = "anonymous";
        img.onload = () => {
          const canvas = document.createElement("canvas");
      ...
      } // end isPlainMode / mapboxToken block
      return;
    }
